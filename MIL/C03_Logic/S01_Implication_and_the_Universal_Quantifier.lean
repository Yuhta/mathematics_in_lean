...
theorem my_lemma4 :
  ...
  calc
    |x * y| = |x| * |y| := abs_mul _ _
    _ ≤ |x| * ε := mul_le_mul_of_nonneg_left (le_of_lt ylt) (abs_nonneg _)
...
example (hfa : FnLb f a) (hgb : FnLb g b) : FnLb (fun x ↦ f x + g x) (a + b) := by
  intro x
  dsimp
  apply add_le_add (hfa _) (hgb _)

example (nnf : FnLb f 0) (nng : FnLb g 0) : FnLb (fun x ↦ f x * g x) 0 := by
  intro x
  dsimp
  apply mul_nonneg (nnf _) (nng _)
...
theorem Subset.trans : r ⊆ s → s ⊆ t → r ⊆ t := fun rs st _ xr ↦ st (rs xr)
