...
theorem neg_eq_of_add_eq_zero {a b : R} (h : a + b = 0) : -a = b := by
  rw [← zero_add (-a), ← h, add_comm a b, add_assoc, add_right_neg, add_zero]

theorem eq_neg_of_add_eq_zero {a b : R} (h : a + b = 0) : a = -b := by
  rw [← zero_add (-b), ← h, add_assoc, add_right_neg, add_zero]
...
theorem mul_inv_rev (a b : G) : (a * b)⁻¹ = b⁻¹ * a⁻¹ := by
  ...
  nth_rw 1 [← one_mul a⁻¹]
  rw [← mul_right_inv b, mul_assoc, ← mul_assoc a, ← mul_assoc, mul_left_inv, one_mul]
